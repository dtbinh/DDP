    ...
    if i<=num_leader  % Movement for leader points
     ...
     %%% Force due repulsion from other robots and other obstacles
       ...
       % settlement_factor=2*settlement_factor_limit; % To ensure all bts have equal chance
        ...
        if length(neigh_index)>=2 % && agent(i).line_spiral==boolean(0)  % what to do if it does find enough neighbours
            agent(i).isspiral=boolean(0);
...
%               if agent(i).line_spiral==boolean(1)  % If agent is in line spiral
%                   all_x=zeros(1,length(agent(i).neighbours));
%                   all_y=zeros(1,length(agent(i).neighbours));
%                 for p=1:length(all_x)
%                     all_x(p)=agent(agent(i).neighbours(p)).present_pos.x-agent(i).present_pos.x;
...
%                 all_x=sort(all_x);
%                 all_y=sort(all_y);
%                 if all_x(1)*all_x(length(all_x))>0 && all_y(1)*all_y(length(all_y))>0  % Checks if agent bot is still at the end of a line  
%                     agent(i).line_spiral=boolean(0);
%                     agent(i).isspiral=boolean(0);
%                     agent(i).spiral_theta=0;
%                     agent(i).radius=0;
%                     agent(i).force=[0,0];
%                 end
%                   
%               end % End of commands if the bot is in a line spiral
