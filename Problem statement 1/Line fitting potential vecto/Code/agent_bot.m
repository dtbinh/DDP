classdef agent_bot
    ...
    properties
        ...
        neigh_old='a'; % Old neighbour kernel
        
        
        isleader=boolean(0); % if zero, means that the agent is a leader bot else follower
        isspiral=boolean(0);  % if 1, means its in a spiral, else zero
        spiral_theta=0; % The angle which tells at what angle it is in the spiral
        radius=0;  % present radius during spiral
        force=0; % Gives the value of the force on the agennt
        settlement_factor=-1; % Stores settlement factor for each agent
        line_spiral=boolean(0); % This value is a binary flag which tells if an agent bot is in a normal spiral or line-ending spiral
        isendline=boolean(0);  % Tells whether an agent is at the end of a line
