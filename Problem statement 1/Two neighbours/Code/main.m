...
for i=1:num_total
    ...
    agent(i).present_pos=updated_pos;    
        ...
        if length(neigh_index)>=2  % what to do if it does find neighbours
            ...
            agent(i).radius=0;
            [sorted, indices]=sort(neigh_dist);
            neighbour1=agent(indices(1));
            neighbour2=agent(indices(2));
            [x1, y1, theta1]=get_coord(neighbour1.present_pos);
            [x2, y2, theta2]=get_coord(neighbour2.present_pos);
            agent(i).desired_pos=([x1,y1,theta1]+[x2,y2,theta2])/2;
